...
in the formacion-raw-data-pipeline.

Responsibilities:
  1. Run pytest on test_formacion.py using subprocess.
  ...
  3. Exit with the same code pytest returned so Kestra marks the task
     correctly (0 = all passed, 1 = failures found).
...
PYTEST_JSON = "pytest_raw_formacion.json"
OUTPUT_JSON = "test_results.json"
OUTPUT_TXT  = "test_report.txt"
TEST_FILE   = "test_formacion.py"
...
def run_pytest() -> int:
    ...
    result = subprocess.run(
        ...
        capture_output=False,   # stream output to Kestra task log in real time
    )
...
def parse_pytest_json(exit_code: int) -> tuple[dict, str]:
    ...
    Read pytest's raw JSON report and reshape it into:
      - summary dict  (written to test_results.json, consumed by Jinja template)
    ...
    itself failed to start or a required import was missing).
    """
    run_at = datetime.now(timezone.utc).isoformat()

    # ── Graceful fallback if pytest never wrote the report ──────────────────
    if not os.path.exists(PYTEST_JSON):
        summary = {
            ...
            "tests":      [],
            "error": (
                "pytest JSON report not found — pytest may have failed to start. "
                "Common causes: formacion.py missing from namespace files; "
                "missing pip dependency (pandas, numpy, gcsfs, openpyxl); "
                "syntax error in test_formacion.py; "
                "ingest_from_gcs or resolve_sheet not exported from formacion.py."
            ),
        }
        report_text = (
            ...
            "Common causes:\n"
            "  - formacion.py not listed in namespaceFiles\n"
            "  - Missing pip dependency (pandas, numpy, gcsfs, openpyxl, etc.)\n"
            "  - Syntax error in test_formacion.py\n"
            "  - resolve_sheet or ingest_from_gcs not exported from formacion.py\n"
        )
    ...
    for item in raw.get("tests", []):
        ...
        # pytest nodeids: test_formacion.py::ClassName::test_method
        # Strip the filename prefix for a cleaner display name in the email.
    ...
    lines = [
        sep,
        "FORMACION DATA QUALITY TEST REPORT",
        f"Run at : {run_at}",
    ...
    lines.append("")
    lines.append(sep)
    lines.append("RESULT: ALL TESTS PASSED" if summary["all_passed"] else f"RESULT: {failed} TEST(S) FAILED")
    lines.append(sep)
...
def main():
    ...
    # Echo to Kestra task log so it's visible in the pipeline UI
    print(report_text)
